...
import 'mentions/mention_santizer.dart';
...
class OpAttributes {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is OpAttributes && runtimeType == other.runtimeType && const DeepCollectionEquality().equals(attrs, other.attrs);
...
class OpAttributeSanitizer {
  static OpAttributes sanitize(
    ...
    const alignments = [AlignType.center, AlignType.right, AlignType.justify, AlignType.left];
    ...
    if (isTruthy(mentions) && isTruthy(mention)) {
      final sanitizedMention = MentionSanitizer.sanitize(mention!, sanitizeOptions);
    ...
    cleanAttrs.attrs.addEntries(dirtyAttrs.attrs.entries.where((entry) => !sanitizedAttrs.contains(entry.key)));
    return cleanAttrs;
  }
  ...
  static bool isValidRGBColor(String colorStr) {
    final re = RegExp(r'^rgb\(((0|25[0-5]|2[0-4]\d|1\d\d|0?\d?\d),\s*){2}(0|25[0-5]|2[0-4]\d|1\d\d|0?\d?\d)\)$', caseSensitive: false);
